...
style.innerHTML = `
  ...
  .tweetmark-btn { width: 100%; padding: 16px; background: #1d9bf0; color: white; border: none; border-radius: 999px; font-weight: 800; font-size: 15px; cursor: pointer; transition: transform 0.1s, background 0.2s, box-shadow 0.2s; margin-top: 4px; box-shadow: 0 4px 12px rgba(29,155,240,0.25); }
  .tweetmark-btn:hover { background: #1a8cd8; box-shadow: 0 6px 16px rgba(29,155,240,0.3); }
...
function showPanel(tweetId) {
  ...
  // Webapp'ten kaydedilen verileri ve pending listeleri oku
  chrome.storage.local.get(['twitter_bookmarks', 'twitter_updates', 'ext_cached_folders', 'ext_cached_tags'], (result) => {
    ...
    panel.querySelector('#tm-save-btn').onclick = () => {
      ...
      chrome.runtime.sendMessage({
        ...
        // Panel kapanırken listener'ları temizle ve kapat
        if (panel.cleanups) {
          panel.cleanups.forEach(cleanup => cleanup());
        }
        panel.classList.remove('show');
  ...
  actionBars.forEach(bar => {
    ...
    if (article) {
      ...
      saveBtnContainer.addEventListener('click', (e) => {
        ...
        // B PLAN (FALLBACK) İÇİN GÖRSEL/VİDEO TOPLAMA
        const isPlayableVideoUrl = (url) => {
          if (!url || typeof url !== 'string' || url.startsWith('blob:')) return false;
          return /(\.m3u8|\.mp4|\.webm|\.ogg|video\.twimg\.com|v\.redd\.it|\/ext_tw_video\/|\/amplify_video\/)/i.test(url);
        };
        const getVideoPriority = (url) => {
          const val = String(url || '').toLowerCase();
          if (/\.(mp4|webm|ogg)(\?|$)/i.test(val)) return 3;
          if (/video\.twimg\.com|\/ext_tw_video\/|\/amplify_video\/|v\.redd\.it/i.test(val)) return 2;
        ...
        try {
          ...
          const isVideoMarker = article.querySelector('[data-testid="videoPlayer"], [data-testid="videoComponent"], [aria-label*="Oynat"], [aria-label*="Play"], [aria-label*="video"], [aria-label*="Video"]');
          const isGifMarker = article.querySelector('[aria-label="GIF"]');

          const playableVideoUrls = [];
          const addPlayableCandidate = (candidate) => {
            const cleaned = String(candidate || '').trim();
            if (!cleaned) return;
            if (isPlayableVideoUrl(cleaned)) playableVideoUrls.push(cleaned);
          };

          // Find specific video elements or markers
          if (videoElement || isVideoMarker || isGifMarker) {
            if (videoElement) {
              if (videoElement.poster && !videoElement.poster.startsWith('blob:')) {
                posterUrl = videoElement.poster;
                fallbackMediaUrls.push(videoElement.poster);
              }
              addPlayableCandidate(videoElement.src);
              addPlayableCandidate(videoElement.currentSrc);
              addPlayableCandidate(videoElement.getAttribute('src'));
              videoElement.querySelectorAll('source').forEach(s => addPlayableCandidate(s.src || s.getAttribute('src')));
            }
          }

          // Scan HTML for ANY hidden video links (can catch videos even if markers are hidden)
          try {
            const html = article.innerHTML || '';
            const patterns = [
              /https?:\/\/video\.twimg\.com\/[^"'\\\s<)]+/g,
              /https?:\/\/[^/]+\.twimg\.com\/amplify_video\/[^"'\\\s<)]+/g,
              /https?:\/\/[^/]+\.twimg\.com\/ext_tw_video\/[^"'\\\s<)]+/g
            ];
            patterns.forEach(p => {
              const matches = html.match(p) || [];
              matches.forEach(m => addPlayableCandidate(m.replace(/\\\//g, '/')));
            });
          } catch (e) { }

          const dedupPlayable = [...new Set(playableVideoUrls)]
            .sort((a, b) => getVideoPriority(b) - getVideoPriority(a));

          if (dedupPlayable.length > 0 || isVideoMarker || isGifMarker || videoElement) {
            fallbackMediaType = 'video';
            if (dedupPlayable.length > 0) {
              fallbackMediaUrls = [...new Set([...dedupPlayable, ...fallbackMediaUrls])];
            }
            // Poster yoksa her zaman bir tane bulmaya çalış
            if (!posterUrl) {
              const posterImg = article.querySelector('[data-testid="tweetPhoto"] img');
              if (posterImg && posterImg.src && !posterImg.src.startsWith('blob:')) {
                posterUrl = posterImg.src;
                if (!fallbackMediaUrls.includes(posterUrl)) fallbackMediaUrls.push(posterUrl);
              }
            }
          } else {
          ...
          fallbackMediaUrls = [...new Set(fallbackMediaUrls.filter(Boolean))];
        } catch (e) { console.error("Media extraction error:", e); }
...
const observer = new MutationObserver((mutations) => {
  ...
  // Optimizing performance for heavy feeds: Throttle with a bigger timeout or requestAnimationFrame
  observerTimeout = setTimeout(() => {
    // Sadece yeni eklenen node'ları analiz etmek daha performanslı olabilir, 
    // ama fallback olarak tüm dökümanı daha seyrek tarıyoruz.
    injectSaveButton();
    observerTimeout = null;
  }, 300); // 100ms yerine 300ms'ye çekildi, CPU yükü ~3 kat azaltıldı
});
